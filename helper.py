...
def most_successful(df, sport):
    ...
    x = x.rename(columns={"index": "Name", "Name_x": "Medals", "region": "Region"})
    return x
...
def most_successful_country_wise(df,country):
    ...
    x = temp_df['Name'].value_counts().reset_index().rename(columns={"Name": "index", "count": "Name"}).head(10).merge(df, left_on='index', right_on='Name', how='left')[['index','Name_x','Sport','region']].drop_duplicates('index')
    x = x.rename(columns={"index":"Name", "Name_x":"Medals", "region":"Region"})
    return x
...
def men_vs_woman(df):
    ...
    woman = athlete_df[athlete_df['Sex'] == 'F'].groupby('Year').count()['Name'].reset_index()

    final = men.merge(woman, on='Year')
